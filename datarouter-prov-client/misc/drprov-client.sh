...
ONBEHALFHDR="X-DMAAP-DR-ON-BEHALF-OF: dcaecm"
...
then
    ...
    do
        ...
        then
            ...
            echo "WARNING: Could not retrieve ${SUBURL}" >&2
        fi
    done < <(echo ${SUBRESP} | jq -r .[])
 else
    echo ERROR: failed to fetch subscription list from "$1" >&2
fi
...
#
    ...
    then
        ...
        then
            ...
            then
                echo "Using existing subscription: ${EXISTINGSUB}.  No new subscription created." >&2
                RV=0
            else
                # Substitute any environment variables in the subscription file
                ...
                then
                    echo "Created new subscription: $(echo ${SUBDATA} | jq -r '.links.self')" >&2
                    RV=0
                fi
            fi
        fi
...
#  any other case.
    ...
    then
        ...
        then
            echo "Using existing feed: $(echo ${FEEDDATA} | jq -r '.links.self'). No new feed created." >&2
            RV=0
        else
            # Create feed
            if FEEDDATA=$(createFeedFromFile "$1")
            then
                echo "Created new feed:  $(echo ${FEEDDATA} | jq -r '.links.self')" >&2
                RV=0
            fi
        fi
    fi
...
#       files defining DR feeds
    ...
    then
        ...
        do
            echo "Creating feed from ${FEEDFILE}"
...
# variables are exported (unlike the provisionFeeds function.)
    ...
    then
        ...
        do
            echo "Creating subscription from ${SUBFILE}"
...
# $2 -- path to application configuration directory
    ...
    do
        echo "Substituting environment vars in ${CONFFILE}"
