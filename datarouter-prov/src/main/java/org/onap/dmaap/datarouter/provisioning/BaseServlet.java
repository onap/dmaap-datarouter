...
/*            if (nodes[i].indexOf('.') < 0) {
  ...
  private void checkHttpsRelaxation() {
    if (!mailSendFlag) {
      ...
      intlogger.info(
          "HTTPS relaxatio: " + p.get("org.onap.dmaap.datarouter.provserver.https.relaxation"));
  ...
  private void notifyPSTeam(String email) throws Exception {
    ...
    mailproperties
        .put("mail.transport.protocol", mailprops.get("com.att.dmaap.datarouter.mail.protocol"));
    ...
    try {
      ...
      intlogger
          .error("Invalid email address, unable to send https relaxation mail to - : " + email);
