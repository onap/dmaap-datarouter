...
public class DB {
	 ...
	 * Uses mysql_init_0000 to setup the DB.
	 ...
	 * @param connection a DB connection
	 * @param scriptId the number of the mysql_init_NNNN script to run
	 */
	private void runInitScript(Connection connection, int scriptId) {
		...
		try {
			String scriptFile = String.format("%s/mysql_init_%04d", scriptDir, scriptId);
