            ...
            if (!(method.equals("POST"))) { // if request method is PUT method (publish or Feed update) Needs to check for DELETE
                ...
                if ((pathUrl.contains("subscribe"))) {//add subscriber
                    ...
                    if (isAAFFeed(feedId)) {//check if AAF Feed or legacy to add new subscriber
                        if (excludeAAF == null) {
                            String message = "DRProvCadiFilter -Invalid request Header Parmeter exclude_AAF = " + httpRequest.getHeader(BaseServlet.EXCLUDE_AAF_HEADER);
                            elr.setMessage(message);
                    ...
                    if (excludeAAF.equalsIgnoreCase("true")) {//add legacy feed
                        String message = "DRProvCadiFilter - Create new legacy Feed : EXLUDE_AAF = " + excludeAAF;
                        elr.setMessage(message);
                        eventlogger.info(elr);
                        chain.doFilter(request, response);
                    } else {//add AAF Feed
                        String message = "DRProvCadiFilter - Create new AAF Feed : EXLUDE_AAF = " + excludeAAF;
                        elr.setMessage(message);
                        eventlogger.info(elr);
                        super.doFilter(request, response, chain);
                    }
