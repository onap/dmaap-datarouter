...
public class PublishServlet extends BaseServlet {
    ...
    private int checkPath(HttpServletRequest req) {
        ...
        try {
            ...
            String provData = poker.getProvisioningString();
            JSONObject provDataJson = new JSONObject(provData);
            JSONArray feeds = provDataJson.getJSONArray("feeds");
            for (int n = 0; n < feeds.length(); ++n) {
                JSONObject jsonFeed = feeds.getJSONObject(n);
                if (jsonFeed.getInt("feedid") == feedid) {
                    return feedid;
                }
            }
            return -1;
