            ...
            try {
                ...
                if (i == -1 || i == fileid.length() - 1) {
                    logger.info("NODE0112 Rejecting bad URI for DELETE of " + req.getPathInfo() + " from " + req
                            .getRemoteAddr());
                    resp.sendError(HttpServletResponse.SC_NOT_FOUND,
                            "Invalid request URI.  Expecting <subId>/<pubId>.  Possible missing pubId.");
                    eelflogger.info(EelfMsgs.EXIT);
                    return;
                }
                ...
                if (fileToDelete.delete() && metaFileToDelete.delete()) {
                    logger.info("NODE0113 File successfully deleted off Node");
                    resp.setStatus(HttpServletResponse.SC_OK);
                    eelflogger.info(EelfMsgs.EXIT);
                    return;
                } else {
                    logger.error("Unable to delete file " + pubid + " from DR.");
