...
public class NodeServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) {
        ...
        eelflogger.info(EelfMsgs.MESSAGE_WITH_BEHALF_AND_FEEDID, req.getHeader("X-DR-ON-BEHALF-OF"),
                getIdFromPath(req) + "");
        try {
            if (down(resp)) {
                return;
    ...
    private void common(HttpServletRequest req, HttpServletResponse resp, boolean isput)
        ...
        if (fileid.startsWith("/publish/")) {
            ...
            xpubid = req.getHeader("X-DR-PUBLISH-ID");
            ...
            pubid = req.getHeader("X-DR-PUBLISH-ID");
            targets = config.parseRouting(req.getHeader("X-DR-ROUTING"));
        } else {
            logger.info("NODE0105 Rejecting bad URI for PUT or DELETE of " + req.getPathInfo() + " from " + req
                    .getRemoteAddr());
            resp.sendError(HttpServletResponse.SC_NOT_FOUND,
        ...
        if (feedid != null) {
            ...
            resp.setHeader("X-DR-PUBLISH-ID", pubid);
        }
        ...
        try {
            ...
            while (hnames.hasMoreElements()) {
                ...
                if ((isput && ("content-type".equals(hnlc) ||
                        ...
                        "x-dr-meta".equals(hnlc) ||
                        (feedid == null && "x-dr-received".equals(hnlc)) ||
                        (hnlc.startsWith("x-") && !hnlc.startsWith("x-dr-"))) {
            ...
            mx.append("X-DR-RECEIVED\t").append(rcvd).append('\n');
            ...
            for (Target t : targets) {
                ...
                mw.write(metadata);
                if (di.getSubId() == null) {
                    mw.write("X-DR-ROUTING\t" + t.getRouting() + "\n");
                }
                mw.close();
